...
void bumpintoworld(float *posx,float *posz,float radius){
	//some variables that will be needed
	int xgridpos=(int)floor(*posx);
	int zgridpos=(int)floor(*posz);
	int range=(int)floor(radius)+1;
	//bump into the sides of blocks
	//bump into corners of blocks
	for(int x=highint(xgridpos-range,0-1); x<lowint(xgridpos+range+1,maxtracksizex+1); x++)
	for(int z=highint(zgridpos-range,0-1); z<lowint(zgridpos+range+1,maxtracksizez+1); z++)
	if(x<0 || x==maxtracksizex || z<0 || z==maxtracksizez || track[x][z]==0)
		bumpintocornerofblock(posx,posz,radius,(float)x-0.6f,1.2f,(float)z-0.6f,1.2f);
}
