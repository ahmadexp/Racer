...
float dash_bars_gameversion=0.1;
...
#ifdef _WIN32
	sprintf_s(dash_bars_credits[0],sizeof(dash_bars_credits[0]),"%s","");
	sprintf_s(dash_bars_credits[1],sizeof(dash_bars_credits[1]),"%s","The Kinetica Game Engine");
#else
	snprintf(dash_bars_credits[0],sizeof(dash_bars_credits[0]),"%s","");
	snprintf(dash_bars_credits[1],sizeof(dash_bars_credits[1]),"%s","The Kinetica Game Engine");
#endif
}
...
void dashbars(){
	...
	char temptext[256];
#ifdef _WIN32
	sprintf_s(temptext,sizeof(temptext),"%s [v%.1f]",dash_bars_gamename,dash_bars_gameversion);
#else
	snprintf(temptext,sizeof(temptext),"%s [v%.1f]",dash_bars_gamename,dash_bars_gameversion);
#endif
