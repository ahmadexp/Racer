void dashsavescreenshot(){
	...
	int filenumber=1;
#ifdef _WIN32
	sprintf_s(filename,sizeof(filename),"screenshot%d.bmp",filenumber);
#else
	snprintf(filename,sizeof(filename),"screenshot%d.bmp",filenumber);
#endif
...
void dashmenu(){
	...
	switch(menu_tree_par){
		case 1:
			...
			y+=40;
			if(dashshowframerate==0){
				if(dashbutton(x,y,200,15,1,1,(char*)"Info Enable"))dashshowframerate=1;
			y+=40;
			}else{
				if(dashbutton(x,y,200,15,1,1,(char*)"Info Disable"))dashshowframerate=0;
			y+=40;
			}
