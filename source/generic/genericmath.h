...
float taxicabdist2d(float x,float y,float x2,float y2){
    return posfloat(x-x2)+posfloat(y-y2);
}

float taxicabdist3d(float x,float y,float z,float x2,float y2,float z2){
    return posfloat(x-x2)+posfloat(y-y2)+posfloat(z-z2);
}
...
float angledifference(float startangle,float endangle){
	while(startangle<endangle-180.f*(float)radiansindegree)startangle+=360.f*(float)radiansindegree;
	while(startangle>endangle+180.f*(float)radiansindegree)startangle-=360.f*(float)radiansindegree;
...
float randangle(){
	float angle=(float)(rand()%(360*100))*0.01f*(float)radiansindegree;
	return angle;
}
